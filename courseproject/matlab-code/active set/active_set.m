...
% 绘制x=0
x_zero = linspace(-3, 3, 100); % x轴的范围
y_zero = zeros(size(x_zero)); % y轴对应的值
plot(x_zero, y_zero, 'k--'); % 绘制x=0的直线
hold on;
% 绘制y=0
y_zero = linspace(-3, 3, 100); % x轴的范围
x_zero = zeros(size(y_zero)); % x轴对应的值
plot(x_zero, y_zero, 'k--'); % 绘制y=0的直线
hold on;
% 绘制ax + by = 0
a = Ai(1,1);b = Ai(1,2);m = bi(1);
y_values = linspace(-3, 3, 100); % x轴的范围
x_values = (m-b*y_values)/a; % 对应的y值
plot(x_values, y_values, 'k--'); % 绘制直线ax + by = 0
